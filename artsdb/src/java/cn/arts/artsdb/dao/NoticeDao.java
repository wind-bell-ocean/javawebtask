...
import cn.arts.artsdb.domain.Notice;
import cn.arts.artsdb.utils.DataSourceUtils;

public class NoticeDao {
	//后台系统，查询所有的公告
	public List<Notice> getAllNotices() throws SQLException {
		String sql = "select * from notice order by notice_time desc limit 0,10";
	...
	public void addNotice(Notice n) throws SQLException {
		String sql = "insert into notice(title,details,notice_time) values(?,?,?)";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
	...
	public void updateNotice(Notice n) throws SQLException {
		String sql = "update notice set title=?,details=?,notice_time=? where notice_id=?";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
