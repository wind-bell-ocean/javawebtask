...
import cn.arts.cyyarts.exception.AddProductException;
import cn.arts.cyyarts.exception.FindProductByIdException;
import cn.arts.cyyarts.exception.ListProductException;

public class ProductService {
	private ArtsDao dao = new ArtsDao();
	//添加商品
	public void addProduct(Arts a) throws AddProductException {
		try {
			dao.addProduct(a);
	...
	//查找所有商品信息；
	public List<Arts> listAll() throws ListProductException {
		try {
			return dao.listAll();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public PageBean findProductByPage(int currentPage, int currentCount,
		...
		try {
			...
			// 获取当前页数据
			List<Arts> ps = dao.findByPage(currentPage, currentCount,
					category);
			bean.setPs(ps);
		} catch (SQLException e) {
			e.printStackTrace();
	...
	// 根据id查找商品
	public Arts findProductById(String id) throws FindProductByIdException {
		try {
	...
	public List<Object[]> download(String year, String month) {
		...
		try {
			salesList = dao.saleList(year, month);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return salesList;
	...
	// 修改商品信息
	public void editProduct(Arts a) {
		try {
			dao.editProduct(a);
		} catch (SQLException e) {
			e.printStackTrace();
	...
	//前台，用于搜索框根据商品名来模糊查询相应的手工艺品
	public PageBean findArtsByName(int currentPage, int currentCount,
			String searchfield) {
		PageBean bean = new PageBean();
		// 封装每页显示数据条数
		bean.setCurrentCount(currentCount);
		...
		try {
			...
			//满足条件的图书
			List<Arts> ps = dao.findArtsByName(currentPage,currentCount,searchfield);
			bean.setPs(ps);
			return bean;
		} catch (SQLException e) {
			e.printStackTrace();
			throw new RuntimeException("前台搜索框根据商品名查询手工艺品失败！");
		}
	}
	//后台系统，根据id删除商品信息
